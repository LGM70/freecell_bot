...
class TestEnv(unittest.TestCase):
    ...
    def test_everything_in_one_episode(self):
        ...
        # cascade and cascade
        _, _, _, _, result = self.env.step(np.array([1, 0], dtype=np.int8))
        self.assertEqual(result['status'], 'success')
        _, _, _, _, result = self.env.step(np.array([3, 1], dtype=np.int8))
        self.assertEqual(result['status'], 'failure')
        _, _, _, _, result = self.env.step(np.array([1, 2], dtype=np.int8))
        self.assertEqual(result['status'], 'success')
        _, _, _, _, result = self.env.step(np.array([1, 2], dtype=np.int8))
        self.assertEqual(result['status'], 'failure')
        # cascade and cell
        _, _, _, _, result = self.env.step(np.array([3, 7], dtype=np.int8))
        self.assertEqual(result['status'], 'success')
        _, _, _, _, result = self.env.step(np.array([2, 5], dtype=np.int8))
        self.assertEqual(result['status'], 'failure')
        _, _, _, _, result = self.env.step(np.array([1, 6], dtype=np.int8))
        self.assertEqual(result['status'], 'failure')
        _, _, _, _, result = self.env.step(np.array([6, 0], dtype=np.int8))
        self.assertEqual(result['status'], 'failure')
        _, _, _, _, result = self.env.step(np.array([5, 0], dtype=np.int8))
        self.assertEqual(result['status'], 'failure')
        _, _, _, _, result = self.env.step(np.array([5, 1], dtype=np.int8))
        self.assertEqual(result['status'], 'success')
        # cascade and foundation
        _, _, _, _, result = self.env.step(np.array([11, 3], dtype=np.int8))
        self.assertEqual(result['status'], 'failure')
        _, _, _, _, result = self.env.step(np.array([0, 10], dtype=np.int8))
        self.assertEqual(result['status'], 'failure')
        _, _, _, _, result = self.env.step(np.array([0, 9], dtype=np.int8))
        self.assertEqual(result['status'], 'success')
        # cell and foundation
        _, _, _, _, result = self.env.step(np.array([8, 5], dtype=np.int8))
        self.assertEqual(result['status'], 'success')
        _, _, _, _, result = self.env.step(np.array([7, 9], dtype=np.int8))
        self.assertEqual(result['status'], 'success')
        # finish the game
        self.env.step(np.array([5, 8], dtype=np.int8))
        self.env.step(np.array([3, 11], dtype=np.int8))
        self.env.step(np.array([1, 11], dtype=np.int8))
        self.env.step(np.array([3, 10], dtype=np.int8))
        self.env.step(np.array([0, 10], dtype=np.int8))
        self.env.step(np.array([0, 9], dtype=np.int8))
        self.env.step(np.array([2, 9], dtype=np.int8))
        self.env.step(np.array([4, 10], dtype=np.int8))
        _, _, terminated, _, _ = self.env.step(np.array([0, 10], dtype=np.int8))
